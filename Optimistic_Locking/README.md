...
If two threads tries to do some operation at the same time, then one succeds other one failes

#### Advantages:

Better through put that pessimistic locking.
Low locking overhead. We get to pick how we want to resolve conflicts.

#### DisAdvantages:

Non trivial and verbose implementation
Not meant for all usescases, esp when contention and conflicts are high
